...
export default function Notifications(){
  ...
  if(error)return <div className="empty-state mx-auto max-w-xl"><span className="mx-auto grid size-14 place-items-center rounded-2xl bg-indigo-100 text-indigo-600 dark:bg-indigo-950 dark:text-indigo-300"><Bell/></span><h1 className="mt-5 text-2xl font-black">Your inbox is private</h1><p className="mt-2 text-slate-500">Log in to see activity related to your sets.</p><Link href="/login" className="btn-primary mt-5">Log in</Link></div>;

  return <div className="mx-auto max-w-4xl">
    <section className="relative overflow-hidden rounded-[2rem] border border-indigo-200/60 bg-gradient-to-br from-indigo-50 via-white to-violet-50 p-7 sm:flex sm:items-end sm:justify-between sm:p-10 dark:border-indigo-900 dark:from-indigo-950/60 dark:via-slate-900 dark:to-violet-950/40"><div className="relative z-10"><p className="eyebrow">Activity center</p><h1 className="mt-3 text-4xl font-black tracking-[-.05em] sm:text-5xl">Your inbox</h1><p className="mt-3 text-slate-500"><strong className="text-indigo-600 dark:text-indigo-300">{data?.unreadCount??0} unread</strong> · {data?.total??0} recent updates</p></div><button disabled={busy==="all"||!data?.unreadCount} onClick={readAll} className="btn-secondary relative z-10 mt-6 sm:mt-0"><CheckCheck size={17}/><ActionLabel busy={busy==="all"} busyText="Marking…">Mark all read</ActionLabel></button><div className="absolute -right-16 -top-24 size-72 rounded-full bg-indigo-300/25 blur-3xl"/></section>
