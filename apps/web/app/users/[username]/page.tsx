...
export default function Profile(){
  ...
  const {data,isLoading,error}=useQuery({queryKey:["profile",username],queryFn:()=>api<any>(`/users/${username}`)});
  if(isLoading)return <LoadingPanel title="Loading this profile" detail="Collecting public sets and community activity…"/>;
  if(error||!data?.user)return <div className="panel p-8 text-center"><h1 className="text-2xl font-black">Profile not found</h1></div>;
