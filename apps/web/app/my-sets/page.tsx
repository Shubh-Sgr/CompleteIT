...
export default function MySets(){
  ...
  return <div>
    <section className="relative overflow-hidden rounded-[2rem] border border-indigo-200/60 bg-gradient-to-br from-indigo-50 via-white to-violet-50 p-7 sm:flex sm:items-end sm:justify-between sm:p-10 dark:border-indigo-900 dark:from-indigo-950/60 dark:via-slate-900 dark:to-violet-950/40"><div className="relative z-10"><p className="eyebrow">Your personal library</p><h1 className="display mt-4">My Sets</h1><p className="mt-4 max-w-xl text-slate-500">Everything you are building, following, improving, and ready to share—in one focused place.</p>{data&&<div className="mt-6 flex flex-wrap gap-2"><span className="chip">{owned.length} created</span><span className="chip">{counts.Published} published</span><span className="chip">{following.length} followed</span></div>}</div><Link href="/create" className="btn-primary relative z-10 mt-7 sm:mt-0"><Plus size={18}/>Create a new set</Link><div className="absolute -right-12 -top-20 size-64 rounded-full bg-indigo-300/25 blur-3xl"/></section>
    {error&&<div className="empty-state mx-auto mt-10 max-w-xl"><Layers3 className="mx-auto text-indigo-500" size={36}/><h2 className="mt-4 text-xl font-black">Your library is waiting</h2><p className="mt-2 text-slate-500">Log in to see your sets and everything you follow.</p><Link href="/login" className="btn-primary mt-5">Log in</Link></div>}
    {!error&&<>
      <div className="scroll-row mt-8 flex gap-2 overflow-x-auto pb-3" aria-label="Set filters">{tabs.map(tab=><button type="button" key={tab} aria-pressed={active===tab} onClick={()=>selectTab(tab)} className={`chip shrink-0 whitespace-nowrap !px-3.5 !py-2 ${active===tab?"!border-indigo-400 !bg-indigo-600 !text-white !shadow-lg !shadow-indigo-500/20":""}`}>{tab}<span className={`rounded-full px-1.5 py-0.5 text-[10px] ${active===tab?"bg-white/20":"bg-slate-100 dark:bg-slate-800"}`}>{counts[tab]??0}</span></button>)}</div>
      ...
      {data&&<><div className="mt-8 flex items-center justify-between"><p className="text-sm font-bold text-slate-500">Showing {visible.length} of {totalForView}</p><p className="text-sm font-black text-indigo-600">{active}</p></div><div className="mt-5 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{visible.map((set:any)=><div key={set.id} className="relative"><SetCard set={set}/><div className="absolute left-4 top-4 z-10 rounded-full border border-white/30 bg-black/20 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-white backdrop-blur">{active==="Following"?`@${set.owner.username}`:`${set.visibility.toLowerCase()} · ${set.status.toLowerCase()}`}</div></div>)}</div>{visible.length===0&&<div className="empty-state mt-8"><Archive className="mx-auto text-slate-400" size={34}/><h2 className="mt-4 text-xl font-black">No matching sets</h2><p className="mt-2 text-sm text-slate-500">{query?"Try a different search or clear it.":active==="Following"?"Follow a public set to keep it here.":`Nothing is currently in ${active.toLowerCase()}.`}</p>{(query||active!=="All")&&<button onClick={()=>{setQuery("");selectTab("All")}} className="btn-secondary mt-5">Clear filters</button>}</div>}</>}
    </>}
  </div>;
}
